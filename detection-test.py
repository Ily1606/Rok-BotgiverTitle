...
def scrollDownToFindCoord():
    ...
    if res == -1:
        ...
        actionTitle(infoCoords[res])
def scrollTopToFindCoord():
    global coord_left_message,info, lastText
    coord_message_list = (415,45,1140,895)
    scrollUp()
    img = take_screenshot(device)
    sleep(1)
...
def actionTitle(infoCoord):
    ...
    if len(position):
        ...
        sleep(1.5)
        '''clickToTarget(coord_target_profile_info)
        ...
        chat(" on you")
        chat(" time for duke is 1 min. I'm waiting in 1 min.")
        detectDone()
        scrollTopToFindCoord()
    else:
